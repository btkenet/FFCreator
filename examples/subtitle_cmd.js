...
const {FFCreatorCenter, FFScene, FFAlbum, FFSubtitle, FFText, FFImage, FFCreator} = require('../');
...
const createFFTask = () => {
    //let rows = await query('SELECT * FROM `renpy_contents` WHERE `renpy_episodes_id` = '+process.argv[2])
    //console.log(rows)
    ...
    connection.connect();
    var res = Array()     // 作为方法执行
    connection.query('SELECT * FROM `renpy_contents` WHERE `renpy_episodes_id` = '+process.argv[2]+' limit 15', function (error, results, fields) {
      ...
      results.forEach((item,index) => {
        ...
        // add title text
        const text = new FFText({text: 'BTKE字幕DEMO'+index, x: width / 2, y: 150, fontSize: 40});
  ...
  console.log('tttttttt'+res)
    ...
    //console.log("video id = "+process.argv[2]);
  ...
  // add title text
  const text = new FFText({text: 'BTKE字幕DEMO', x: width / 2, y: 150, fontSize: 40});
  text.setColor('#ffffff');
  text.setBackgroundColor('#019333');
  text.addEffect('fadeInUp', 1, 1);
  text.alignCenter();
